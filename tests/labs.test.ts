/**
 * Programmatic test for all 38 labs.
 *
 * For each lab verifies:
 *  1. Broken starter YAML → at least one objective fails
 *  2. Solution YAML       → all objectives pass
 *
 * Run with: pnpm test
 */
...
import type { VerifierFn } from "@/lib/verifiers/types";

const VERIFIERS: Record<string, VerifierFn> = {
  a1: verifyA1, a2: verifyA2, a3: verifyA3, a4: verifyA4,
  a5: verifyA5, a6: verifyA6, a7: verifyA7, a8: verifyA8,
  b1: verifyB1, b2: verifyB2, b3: verifyB3, b4: verifyB4,
  b5: verifyB5, b6: verifyB6, b7: verifyB7, b8: verifyB8,
  b9: verifyB9, b10: verifyB10,
  c1: verifyC1, c2: verifyC2, c3: verifyC3, c4: verifyC4,
  c5: verifyC5, c6: verifyC6, c7: verifyC7, c8: verifyC8, c9: verifyC9,
  d1: verifyD1, d2: verifyD2, d3: verifyD3, d4: verifyD4,
  d5: verifyD5, d6: verifyD6, d7: verifyD7, d8: verifyD8,
  d9: verifyD9, d10: verifyD10, d11: verifyD11, d12: verifyD12, d13: verifyD13,
};

const LABS_DIR = path.resolve(__dirname, "../content/labs");
const TICKS = 20;

// ── Helpers ───────────────────────────────────────────────────────────────────

function buildSim(hiddenDocs: unknown[], applyYamlStr: string): ClusterSimulator {
  const sim = new ClusterSimulator();
  if (hiddenDocs.length > 0) {
    const hiddenYaml = hiddenDocs.map((d) => yaml.dump(d)).join("---\n");
    sim.apply(hiddenYaml);
  }
  sim.apply(applyYamlStr);
...
describe("Lab suite", () => {
  for (const labId of Object.keys(VERIFIERS)) {
    const labFile = path.join(LABS_DIR, `${labId}.json`);
    const lab = JSON.parse(readFileSync(labFile, "utf8")) as {
      starterYaml: string;
      solution: string;
      hiddenSetup?: unknown[];
    };

    const verifier = VERIFIERS[labId];
    const hidden = lab.hiddenSetup ?? [];

    describe(`Lab ${labId.toUpperCase()}`, () => {
      it("starter YAML causes at least one objective to fail", () => {
        const sim = buildSim(hidden, lab.starterYaml);
        const results = verifier(sim, lab.starterYaml);
        const failed = results.filter((r) => !r.passed);
        expect(
          failed.length,
          `Starter YAML passes ALL objectives — it is too easy (objectives: ${results.map((r) => r.id).join(", ")})`
        ).toBeGreaterThan(0);
      });

      it("solution YAML passes all objectives", () => {
        const sim = buildSim(hidden, lab.solution);
        const results = verifier(sim, lab.solution);
        const failed = results.filter((r) => !r.passed);
        expect(
          failed,
          `Solution fails objectives: ${failed.map((r) => `${r.id} — ${r.hint ?? r.description}`).join(" | ")}`
        ).toHaveLength(0);
      });
    });
  }
